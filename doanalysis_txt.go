package contentanalysis

import (
	"fmt"
	CT "github.com/fbaube/ctoken"
	L "github.com/fbaube/mlog"
	SU "github.com/fbaube/stringutils"
	XU "github.com/fbaube/xmlutils"
	S "strings"
)

// DoAnalysis_txt is called when the content is identified
// as non-XML. It does not expect to see binary content.
// .
func (pCA *ContentAnalysis) DoAnalysis_txt(sCont string) error {
	// Markdown is a tough case, because it's basically a text file.
	// There is no string that definitively declares "This is Markdown",
	// and there might not be YAML metadata at the start of the file, and
	// at this point here we don't want to scan (and try to definitively
	// categorise) ALL the file content, at least not more than the first
	// few characters. So, about the best we can do is check for a known
	// file extensions.
	if S.HasPrefix(pCA.MimeTypeAsSnift, "text/") &&
		SU.IsInSliceIgnoreCase(pCA.ContypingInfo.FileExt, XU.MarkdownFileExtensions) {
		pCA.ContypingInfo.MimeType = "text/markdown"
		pCA.ContypingInfo.MimeTypeAsSnift = "text/markdown"
		pCA.ContypingInfo.MType = "mkdn/tpcOrMap/?fmt"
	} else {
		L.L.Warning("Reached no conclusion about type of non-XML content")
	}
	// L.L.Okay("(CA) Non-XML: " + pCA.ContypingInfo.MultilineString())
	// Check for YAML metadata
	iEnd, e := SU.YamlMetadataHeaderRange(sCont)
	// if there is an error, it will mess up parsing the file, so just exit.
	if e != nil {
		L.L.Error("(CA) Metadata header YAML error: " + e.Error())
		return fmt.Errorf("(CA) metadata header YAML error: %w", e)
	}
	// Default: no YAML metadata found
	pCA.Text.Beg = *CT.NewFilePosition(0)
	pCA.Text.End = *CT.NewFilePosition(len(sCont))
	pCA.Meta.Beg = *CT.NewFilePosition(0)
	pCA.Meta.End = *CT.NewFilePosition(0)
	// No YAML metadata found ?
	if iEnd <= 0 {
		pCA.Meta.Beg = *CT.NewFilePosition(0)
		pCA.Meta.End = *CT.NewFilePosition(0)
		pCA.Text.Beg = *CT.NewFilePosition(0)
		pCA.Text.End = *CT.NewFilePosition(len(sCont))
	} else {
		// Found YAML metadata
		s2 := SU.TrimYamlMetadataDelimiters(sCont[:iEnd])
		ps, e := SU.GetYamlMetadataAsPropSet(s2)
		if e != nil {
			L.L.Error("(CA) loading YAML: " + e.Error())
			return fmt.Errorf("loading YAML: %w", e)
		}
		// SUCCESS! Set ranges.
		pCA.Meta.Beg = *CT.NewFilePosition(0)
		pCA.Meta.End = *CT.NewFilePosition(iEnd)
		pCA.Text.Beg = *CT.NewFilePosition(iEnd)
		pCA.Text.End = *CT.NewFilePosition(len(sCont))

		pCA.MetaProps = ps
		L.L.Debug("(CA) Got YAML metadata: " + s2)
	}
	// s := SU.NormalizeWhitespace(sCont) // pCA.PathProps.Raw)
	// s = SU.TruncateTo(s, 56)
	L.L.Debug("|RAW|%.40s ...|END|", sCont)
	// L.L.Okay("(CA) Success: detected non-XML")
	return nil
}
